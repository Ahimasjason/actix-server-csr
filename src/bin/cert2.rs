...
fn create_ssl_selector() -> SslAcceptorBuilder {
    ...
    acceptor.set_private_key_file("certs/cert1/key.pem", SslFiletype::PEM).unwrap();
    acceptor.set_certificate_file("certs/cert1/cert.pem", SslFiletype::PEM).unwrap();
    acceptor
}
...
async fn main() -> std::io::Result<()>  {
    ...
    HttpServer::new(|| App::new().service(index))
        .bind_openssl("127.0.0.1:8082", create_ssl_selector())?
        .run()
        .await
}
